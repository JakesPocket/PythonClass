...
Description:
    This application simulates Randoms Walks. It accept three command line arguments.
    Depending on what is specified, a result of statistics will be printed and the ending
    coordinate points will be plotted in a popup window.
    
New things I've learned:
    - New algirithms using function calls
    - List comprehension
    - Single line 'for loops' and 'conditionals'
    - The '_' character is preferred if the iterating varable is not needed. example: for _ in range(10):
    - More about conditionals
ReadMe:
    This porgram can accept the following as arguments:
        walkers 				- One walker's name ('Pa', 'Mi-Ma', or 'Reg') OR 'all'. See initialize() for more details.
        walks_per_trial			- An (integer) of how many walks will be simulated per trial.
        list_of_steps_per_trial - Comma separated integers for how many steps. Each integer represents one trial.
    example: "python3 random_walk.py <integer(steps)> <integer(walks)> <walker>"
    
    If you are testing this code, you may want to change the 'testing' boolean to True.
'''
...
def main(): # main() will only be called if this file (random_walk.py) is ran directly
    ...
    try:
        list_of_steps_per_trial = list(map(int,sys.argv[1].split(','))) # Converts 'steps' from string into a list of integers
    ...
    # If any arguement is bad or missing  --- return Error..
        ...
        if testing:
            ...
            print('Error: Ensure you enter "python3 random_walk.py <integer(steps)> <integer(walks)> <walker>".\nFor more details - See Module DOCSTRING')
            return
...
def initialize(walkers, called_from_plot):
    ...
    start_point = [0,0]
    north, east, south, west = ([0, 1],[1, 0],[0, -1],[-1, 0]) if not called_from_plot else ([0, 5],[5, 0],[0, -5],[-5, 0])
    ...
    for who in walkers:                                 			# example: for _pa in [_pa, _ma, reg]:
        for steps_per_walk in list_of_steps_per_trial:				# example: for 100 in [100, 1000]:
            for _ in range(walks_per_trial): 						# example: will iterate 50 time because 'walks_per_trial' is equal to 50.
                ...
                for _ in range(steps_per_walk):						# example: will iterate 100 time because 'steps_per_walk' is equal to 100.
                    ...
                    who['now_point'] = 	[x_cor + y_cor for x_cor, y_cor in zip(who['now_point'], random.choice(who['ways']) )]
                # End - for _ in steps_...
